...
class Game:
    ...
    def __single_turn(self):
        ...
        if re.fullmatch(create_command_regex_verifier(LEGAL_CAR_NAMES, LEGAL_MOVES), inp):
            name, move = inp.split(',')
...
def get_cars_from_json(filename: str) -> List[Car]:
    ...
    for name in contents:
        if name in LEGAL_CAR_NAMES and name not in used_names:
            car_params = contents[name]
            if check_car_params(car_params):
...
if __name__== "__main__":
    ...
    if args and len(args) == 2:
        ...
        board = Board()
        for car in get_cars_from_json(json_path):
            board.add_car(car)
